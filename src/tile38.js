...
class Tile38 {
  ...
  get(key, id, { withfields = false, type = null } = {}) {
    ...
    if (type != null && type.startsWith("HASH")) {
      ...
      params.push("HASH");
      let s = type.split(" ");
      if (s.length > 1 && parseInt(s[1]) > 0) params.push(s[1]);
      else throw new Error('missing precision. Please set like this: "HASH 6"');
    } else if (type != null) {
      params.push(type);
    }
  ...
  openLiveFence(command, commandArr, callback) {
    if (this.debug) {
      console.log(
        `sending live fence command "${command} ${commandArr.join(" ")}"`
      );
    }
    let cmd = this.redisEncodeCommand(command, commandArr);
    return new LiveGeofence(this.debug).open(
      this.host,
      this.port,
      this.password,
      cmd,
